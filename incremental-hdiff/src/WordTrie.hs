...
instance Eq a => Semigroup (Trie a) where
  (<>) = append

instance Eq a => Monoid (Trie a) where
  mempty = empty
...
append :: Eq a => Trie a -> Trie a -> Trie a
append (Fork va ma) (Fork vb mb) = Fork va (M.union ma mb)
  where
    x = if va == vb then va else error "Failed to merge Trie"
